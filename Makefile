...
SHELL=/bin/bash
VERSION=$(shell python -c"import django_currentdistrict as m; print(m.__version__)")
...
# lint:
# 	flake8 django_currentdistrict tests --max-complexity=10
...
coverage:
	coverage run --source django_currentdistrict setup.py test
	coverage report -m
	coverage html
