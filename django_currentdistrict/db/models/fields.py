...
from django_currentdistrict.middleware import get_current_verified_district


class CurrentDistrictField(models.ForeignKey):

    warning = ("You passed an argument to CurrentDistrictField that will be "
               "ignored. Avoid args and following kwargs: default, null, to.")
    description = _(
        'as default value sets the current logged district in context if available')
    defaults = dict(null=True, default=get_current_verified_district,
    ...
    def pre_save(self, model_instance, add):
        if self.on_update:
            value = get_current_verified_district()
